...
import { Notification, NotificationCategory, NotificationDocument, NotificationType } from '../../database/schemas/notification.schema';
...
export class NotificationsService {
  ...
  async checkPendingPayments(): Promise<NotificationDocument[]> {
    ...
    for (const issue of issues) {
      ...
      if (due > 1000) { // More than 1000 tk due
        const existing = await this.notificationModel.findOne({
